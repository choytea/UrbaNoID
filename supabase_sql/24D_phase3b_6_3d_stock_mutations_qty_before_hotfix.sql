...
-- PHASE 3B.6.3D: Stock Mutations qty_before / qty_after Compatibility Hotfix
-- ============================================================
-- Perbaikan error checkout:
-- null value in column "qty_before" of relation "stock_mutations" violates not-null constraint
--
-- Penyebab:
-- Database lama memiliki kolom stock_mutations.qty_before NOT NULL
-- dan kemungkinan qty_after NOT NULL, sedangkan function buyer_checkout
-- sebelumnya baru mengisi before_qty / after_qty.
--
-- Dampak:
-- 1. Menjamin kolom legacy qty_before, qty_after, qty_delta tersedia dan aman.
-- 2. Rebuild buyer_checkout agar insert stock_mutations mengisi:
--    before_qty, after_qty, qty_before, qty_after, quantity, qty_delta.
-- 3. Tetap membawa fix order_no, payment_status, product_id, dan Biteship.
-- ============================================================
...
-- ------------------------------------------------------------
-- 2. Tambahkan kolom mutasi stok baru dan legacy.
-- ------------------------------------------------------------
...
alter table public.stock_mutations add column if not exists qty_before integer;
alter table public.stock_mutations add column if not exists qty_after integer;
...
set quantity = coalesce(quantity, abs(qty_delta), 0),
    qty_delta = coalesce(qty_delta, case when quantity is null then 0 else -abs(quantity) end, 0),
    before_qty = coalesce(before_qty, qty_before, 0),
    after_qty = coalesce(after_qty, qty_after, coalesce(qty_before, 0) + coalesce(qty_delta, 0), 0),
    qty_before = coalesce(qty_before, before_qty, 0),
    qty_after = coalesce(qty_after, after_qty, coalesce(qty_before, before_qty, 0) + coalesce(qty_delta, 0), 0),
    created_at = coalesce(created_at, now())
where quantity is null
   or qty_delta is null
   or before_qty is null
   or after_qty is null
   or qty_before is null
   or qty_after is null
...
begin
  ...
  begin
    alter table public.stock_mutations alter column before_qty set default 0;
  exception when others then null;
  end;

  begin
    alter table public.stock_mutations alter column after_qty set default 0;
  exception when others then null;
  end;

  begin
    alter table public.stock_mutations alter column qty_before set default 0;
  exception when others then null;
  end;

  begin
    alter table public.stock_mutations alter column qty_after set default 0;
  exception when others then null;
  end;
end $$;
...
select 'phase_3b_6_3d_stock_mutations_qty_before_hotfix_ready' as status, now() as executed_at;