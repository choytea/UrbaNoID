...
-- PHASE 3B.6.3B: Stock Mutations Enum Compatibility Hotfix
-- ============================================================
-- Perbaikan error SQL sebelumnya dan checkout:
-- column "product_id" of relation "stock_mutations" does not exist
-- column "mutation_type" is of type stock_mutation_type but expression is of type text
--
-- Penyebab:
-- Database lama sudah memiliki tabel stock_mutations, tetapi belum memiliki
-- kolom product_id/variant_id/field mutasi yang dipakai function checkout baru.
--
-- Dampak:
-- 1. Menjamin tabel stock_mutations dan semua kolom yang dibutuhkan tersedia.
-- 2. Menjaga kompatibilitas dengan skema lama tanpa menghapus data.
-- 3. Rebuild buyer_checkout agar tetap mengisi order_number + order_no,
--    payments, shipments, dan stock_mutations.
-- ============================================================
...
-- PERBAIKAN 3B.6.3B:
-- Jangan assign mutation_type::text ke kolom enum stock_mutation_type.
-- Cukup amankan kolom numeric/timestamp; checkout akan selalu mengisi mutation_type eksplisit.
...
-- Jika kolom quantity sebelumnya nullable, beri default aman.
do $$
begin
  begin
...
-- ------------------------------------------------------------
-- 3. Jika mutation_type memakai enum lama, tambahkan nilai OUT_SALE.
--    Nilai enum tidak dipakai untuk UPDATE pada script ini agar aman.
--    Function checkout akan memakai nilai ini setelah script selesai/commit.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4. Pastikan kolom legacy order_no dan status enum sebelumnya tetap aman.
-- ------------------------------------------------------------
...
-- Enum status Indonesia untuk tabel order/payment/shipment.
do $$
declare
  enum_rec record;
  status_value text;
...
-- ------------------------------------------------------------
-- 5. Pastikan kolom pendukung checkout/Biteship tetap tersedia.
-- ------------------------------------------------------------
...
begin
  ...
  loop
    ...
    -- Catat mutasi stok. Kolom sudah dijamin tersedia oleh patch ini.
...
select 'phase_3b_6_3b_stock_mutations_enum_hotfix_ready' as status, now() as executed_at;