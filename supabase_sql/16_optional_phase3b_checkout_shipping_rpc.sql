-- ============================================================
-- OPTIONAL Phase 3B Patch
-- Update buyer_checkout to store expedition metadata in shipments.
-- Jalankan hanya jika ingin data ekspedisi tampil lengkap di Pesanan Seller.
-- Aman dijalankan setelah Phase 3A dan Phase 3B Foundation.
-- ============================================================
...
begin
  ...
  loop
    ...
    select stock_qty into v_before_qty
    ...
    set stock_qty = v_after_qty, updated_at = now()
    where id = v_variant_id;
  ...
  insert into public.payments (order_id, payment_method, payment_status, amount, created_at)
  values (v_order_id, coalesce(nullif(trim(p_payment_method), ''), 'BANK_TRANSFER'), 'BELUM_DIBAYAR', v_grand_total, now());
...
select 'phase_3b_checkout_shipping_rpc_ready' as status, now() as executed_at;
