...
-- PHASE 3B.6.1: Payment Status Enum Fix + Store Origin Location
-- ============================================================
-- Perbaikan error:
-- invalid input value for enum payment_status: "BELUM_DIBAYAR"
--
-- Penyebab:
-- Kolom payment_status memakai enum lama, sedangkan checkout memakai status Indonesia.
--
-- Dampak patch:
-- 1. Menambah label status Indonesia ke enum status yang melekat di orders/payments/shipments.
-- 2. Menambah data origin Biteship di store_profiles: area_id, location_id, latitude, longitude.
-- 3. Menambah field destination area/coordinate untuk pesanan/shipment sebagai fondasi tarif akurat.
-- 4. Rebuild buyer_checkout agar tetap sinkron dengan dimensi paket dan status checkout.
-- ============================================================

-- ------------------------------------------------------------
-- 1. Tambahkan nilai enum status secara aman.
--    Tanpa BEGIN agar nilai enum langsung committed per statement
--    pada SQL Editor.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2. Store Profile: origin Biteship lebih akurat.
-- ------------------------------------------------------------
...
alter table public.store_profiles add column if not exists origin_latitude numeric(12,8);
alter table public.store_profiles add column if not exists origin_longitude numeric(12,8);
...
-- ------------------------------------------------------------
-- 3. Destination support untuk fase cek ongkir real-time berikutnya.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4. Pastikan struktur order_items/shipments Biteship lengkap.
-- ------------------------------------------------------------
...
--    Setelah enum diperbaiki, status BELUM_DIBAYAR valid.
-- ------------------------------------------------------------
...
begin
  ...
  insert into public.orders (
    ...
    destination_area_id, destination_latitude, destination_longitude,
...
select 'phase_3b_6_1_payment_enum_origin_location_ready' as status, now() as executed_at;
